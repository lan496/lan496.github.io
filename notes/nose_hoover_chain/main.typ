...
$
Generalized Liouville's Theorem states that
$
  e^( -w(bold(x)_t) ) d bold(x)_t = e^( -w(bold(x)_0) ) d bold(x)_0.
$
...
$
  ...
  dot(p)_(eta_M) &= G_M \
  dot(p)_(xi_j) &= G'_j - frac( p_(xi_(j+1)), Q'_(j+1) ) p_(xi) quad (j = 1, dots, M-1) \
  dot(p)_(xi_M) &= G'_M \
$
where
$
...
$
  cal(H)' := cal(H) (r, p) + frac( p_epsilon^2, 2W) + P V
...
The conserved energy
$
  cal(H)' &:= cal(H) (r, p) + frac( "Tr"[bold("p ")_g^top bold("p ")_g], 2 W_g) + P det[bold("h ")] \
